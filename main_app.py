...
def save_vector_store(vector_store, path=FAISS_INDEX_PATH):
    vector_store.save_local(path)

# Function to load FAISS index
def load_vector_store(path=FAISS_INDEX_PATH):
    if os.path.exists(path):
        return FAISS.load_local(path, GoogleGenerativeAIEmbeddings(model="models/embedding-001"))
    return None

# Function to fetch dataset (cached for efficiency)
@lru_cache(maxsize=10)
def fetch_dataset(input_query):
    url = "https://researcher-agent-df3x.onrender.com/process_dataset/"
    response = requests.post(url, json={"input": input_query})
...
def process_urls(urls):
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        results = executor.map(fetch_and_split, urls)
    ...
    if all_docs:
        ...
        vector_store = FAISS.from_documents(all_docs, embeddings)
        save_vector_store(vector_store)  # Save for future use
        return vector_store
    return None
...
def run():
    ...
    # Button to fetch dataset and create RAG
    if st.button("Fetch Dataset and Create RAG"):
        if user_input.strip():
            try:
                with st.spinner("Fetching datasets and creating RAG system..."):
                    response_json = fetch_dataset(user_input)
    ...
    # Display current dataset URLs
    if st.session_state.current_links:
        st.subheader("Current Dataset URLs:")
        for link in st.session_state.current_links:
            st.markdown(f"- {link}")

    # Question answering (only if vector store exists)
    if st.session_state.vector_store:
        st.subheader("Ask Questions About the Dataset")
        ...
        document_chain = create_stuff_documents_chain(llm, prompt)
        retrieval = st.session_state.vector_store.as_retriever(search_kwargs={"k": 10})
        retrieval_chain = create_retrieval_chain(retrieval, document_chain)
